...
export class SubscriptionService {
    ...
    constructor(
        ...
        private readonly hostBalancerService: HostBalancerService,
    ) {
    ...
    > {
        try {
            ...
            const balancedHosts = await this.applyHostBalancersIfEnabled(
                user.response,
                hosts.response,
            );
    ...
    public async getRawSubscriptionByShortUuid(
        ...
        try {
            ...
            if (!hwidCheckup || hwidCheckup.subscriptionAllowed) {
                ...
                const balancedHosts = await this.applyHostBalancersIfEnabled(user, hosts.response);
    ...
    private async applyHostBalancersIfEnabled(
        user: UserEntity,
        hosts: HostWithRawInbound[],
    ): Promise<HostWithRawInbound[]> {
        if (!this.configService.get('HOST_BALANCER_ENABLED')) {
            return hosts;
        }
        ...
        try {
            ...
            return await this.hostBalancerService.applyToHostsForUser(user, hosts);
        } catch (error) {
            this.logger.error(`Host Balancer failed open: ${error}`);
            return hosts;
        }
    }
